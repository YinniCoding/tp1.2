...
<div id="page-wrapper">
	...
	<div class="row">
		...
		<div class="col-md-6">
			<br><br>
			<a href="<?php echo U('post/add');?>" class="btn btn-primary">添加文章</a>
		</div>
		<div class="col-md-6">
			<br><br>
			<form action="<?php echo U('post/index');?>" method="post">
				<div class="form-group input-group">
					<input type="text" class="form-control" name="key" placeholder="输入文章标题进行检索…">
	...
	<table class="table table-hover table-striped">
		...
		<tbody>
			<?php if(is_array($model)): foreach($model as $key=>$v): ?><tr>
					...
					<?php if($v["status"] == 1): ?><td><a href="<?php echo U('post/push?id='); echo ($v["id"]); ?>" style="color:red;">撤销</a> | <a href="<?php echo U('post/update?id='); echo ($v["id"]); ?>">编辑</a> | <a href="<?php echo U('post/delete?id='); echo ($v["id"]); ?>" style="color:red;" onclick="javascript:return del('您真的确定要删除吗？\n\n删除后将不能恢复!');">删除</a></td>
						<?php else: ?>
						<td><a href="<?php echo U('post/push?id='); echo ($v["id"]); ?>" style="color:#50AD1E;">发布</a> | <a href="<?php echo U('post/update?id='); echo ($v["id"]); ?>">编辑</a> | <a href="<?php echo U('post/delete?id='); echo ($v["id"]); ?>" style="color:red;" onclick="javascript:return del('您真的确定要删除吗？\n\n删除后将不能恢复!');">删除</a></td><?php endif; ?>
